...
window.format = number => math.format(number, {
    precision: 7
})
...
window.animateElement = async (element) => {
...
window.animateDigits = () => $$('.animate').forEach(async (element) => animateElement(element));
window.convertTZ = (date, tzString) => new Date((typeof date === "string" ? new Date(date) : date).toLocaleString("en-US", {
    timeZone: tzString
}));
window.plural = function(number, versions, options = {
    showNumber: true,
    localize: true,
    locale: null
}) {
